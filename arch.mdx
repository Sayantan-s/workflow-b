...
vee-validate with zod
from -> input [email]
to -> tags-input -> you can build a seperate component for that

{" "}

<EmailSender /> (can change the naming according to use cases.) - normal email
tags - cc tags - bcc tags [build them according to better use case] subject:
input[text] body: richtexteditor (have install quill for that)
...
delay
vee-validate with zod
Delay Duration -> -> number input -> counter [ - ] <value> [ + ]
Metrics
method -> Select [seconds, minutes, hours, day]
